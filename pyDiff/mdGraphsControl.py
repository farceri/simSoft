...
if __name__ == '__main__':
                    ...
                    for tt, tau in enumerate(taus):
                        for integrator in integrators:
                            for interaction in interactions:
                                ...
                                if active: 
                                    if mixture:
                                        if interaction : optionsDirectory = home + os.sep + f"{integrator}WCA_N{num_particles:d}_phi{0.5:.1f}_tau{tau:.1f}_v01{activity:.1f}_v02{(ratio*activity):.1f}_g{gamma:.2f}"
                                        else : optionsDirectory = home + os.sep + f"{integrator}FREE_N{num_particles:d}_tau{tau:.1f}_v01{activity:.1f}_v02{(ratio*activity):.1f}_g{gamma:.2f}"
                                    else:
    ...
    giantIdx = cluster(directory, "gcc", start=100, stop=100, howMany=1, plot=True, densityStudies=True)
