...
#np.random.seed(0)
# python main.py '/home/auroisflying/thesis/simSoft/pyDiff/test' 1e07 3e03 0.5 3 1 10 0 em WCAnumba
                ...
                for kk, gamma in enumerate(frictions):
                    for tau in taus:
                        ...
                        for tau_ratio in tau_ratios:
                            ...
                            if active: 
                                if mixture:
                                    if interaction : optionsDirectory = f"{integrator}WCA_N{num_particles:d}_phi{packingFraction:.1f}_tau1{tau:.1f}_tau2{tau*tau_ratio:.1f}_v01{v0:.1f}_v02{v0*activity_ratio:.1f}_g{gamma:.2f}"
                                    else : optionsDirectory = f"{integrator}FREE_N{num_particles:d}_tau1{tau:.1f}_tau2{tau*tau_ratio:.1f}_v01{v0:.1f}_v02{v0*activity_ratio:.1f}_g{gamma:.2f}"
                                else:
                            ...
                            for iteration in range(iterations):
                                ...
                                md = MolecularDynamics(num_particles, temperature, gamma, potentialType, interaction=interaction, 
                                                    ...
                                                    tau = tau, activity_ratio = activity_ratio, tau_ratio = tau_ratio, activity = activity, active = active)
                            ...
                            if compute_gif: 
                                ...
                                part_evolution(md, md.allPositions, step=1)
